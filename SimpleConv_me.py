...
def main():
    ...
    filter_layer = Conv2D(1, kernal_size=3, 
    ...
    while key == -1:
        ...
        sobel_x_frame = cv2.Sobel(gray_frame, cv2.CV_32F, 
                                  1, 0, ksize=3, scale=.25)
        

        batch_input = np.expand_dims(gray_frame, axis=0) # remember input is color, output is grayscale image
